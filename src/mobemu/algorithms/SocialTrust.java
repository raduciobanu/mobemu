...
/**
 * Class for a node running the Social Trust algorithm (from "Social-Based Trust
 * in Mobile Opportunistic Networks" by Mtibaa and Harras).
 *
 * @author Radu
 */
public class SocialTrust extends Node {
    ...
    private void epidemicDataExchange(Node encounteredNode, long contactDuration, long currentTime) {
        ...
        for (Message message : encounteredNodeSocialTrust.dataMemory) {
            ...
            /*
             * this is the extra trust condition (if the encountered node trusts
             * the current node or the source of the message, it delivers the
             * data)
             */
        ...
        // TODO: should I carry a node's messages if it is not to be trusted?
        for (Message message : encounteredNodeSocialTrust.ownMessages) {
            if (totalMessages >= remainingMessages) {
                return;
            }
    ...
    private boolean isTrusted(SocialTrust encounteredNode, Message message, Long currentTime) {
        ...
        switch (filterType) {
            ...
            /**
             * for the common friends filter, check if the number of common
             * social network friends between the carrier node and the
             * encountered node of the source of the message is higher than the
             * specified limit
             */
            case COMMON_FRIENDS:
