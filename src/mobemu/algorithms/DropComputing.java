...
public class DropComputing extends Node {
     ...
     * @param deviceType type of device for this node
     * @param sampleTime sample time of the trace
     * @param opportunistic specifies whether a node employs other nodes for
     * task computation or not
     * @param useCloud specifies whether a node employs other nodes for task
     * computation or not
     */
    public DropComputing(int id, int nodes, Context context, boolean[] socialNetwork, int dataMemorySize,
        ...
        this.battery = new Battery(RAND.nextDouble() * deviceInfo.fullChargeDuration / sampleTime,
                deviceInfo.fullChargeDuration / sampleTime,
                deviceInfo.rechargeDuration / sampleTime, 0.2);
    }
    ...
    protected void onDataExchange(Node encounteredNode, long contactDuration, long currentTime) {
        ...
        boolean condition = (encounteredNodes.get(dcNode.id) != null && encounteredNodes.get(dcNode.id).getContacts() >= 2);
        ...
        if (battery.canParticipate() && dcNode.battery.canParticipate()) {
            ...
            if (exchangedData && ownTasks.hasFinished()) {
                ...
                // compute statistics (increase the number of completed task groups and the total
            ...
            // remove all completion messages from encountered node, so they won't be disseminated more
     ...
     * @param sampleTime sample time of the trace
     */
    private static void setBatteryOpportunisticExchange(DropComputing first, DropComputing second,
            TaskType type, long sampleTime) {
     ...
     * @param first first node
     * @param second second node
     * @param tasksFirst tasks of the first node
     * @param tasksSecond tasks of the second node
     */
    private static void balance(final DropComputing first, final DropComputing second,
            List<Task> tasksFirst, List<Task> tasksSecond) {
        ...
        Collections.sort(tasksFirst, new Comparator<Task>() {
            ...
            public int compare(Task o1, Task o2) {
                ...
                return (int) (firstBoost + o1.computeExecutionTime(first.deviceInfo)
                        - secondBoost - o2.computeExecutionTime((first.deviceInfo)));
            }
        });
        ...
        Collections.sort(tasksSecond, new Comparator<Task>() {
            ...
            public int compare(Task o1, Task o2) {
                ...
                return (int) (firstBoost + o1.computeExecutionTime(second.deviceInfo)
                        - secondBoost - o2.computeExecutionTime((second.deviceInfo)));
            }
        });
    ...
    protected void onTick(long currentTime, long sampleTime) {
        ...
        // compute status (inscrease the maximum number of messages stored per node
    ...
    private static class DropComputingCloud {
         ...
         *
         * @param currentTime current trace time
         * @param sampleTime duration between two execution samples
         */
         ...
         *
         * @param group task group to check
         ...
         *
         * @param group task group whose result is wanted
    ...
    private static class DeviceFactory {
         ...
         *
         * @param type type of the object to be instantiated.
    ...
    private static abstract class DeviceInfo {
         ...
         * transfer.
         *
         * @param wifi {@code true} if the transfer is done using WiFi,
    ...
    private class TaskGroup implements Comparable<TaskGroup> {
         ...
         *
         * @param tasks set of tasks to be added
         */
         ...
         *
         * @param other task group where the new tasks are taken from
         */
         ...
         *
         * @param timestamp timestamp when this task group was finished
         */
         ...
         *
         * @param device device that executes the task group
         * @param sampleTime time between two execution samples
         * @param executorID ID of the executing node
         ...
         * the cloud.
         *
         * @param device device that uploads the task group
         * @param sampleTime sample time of the trace
    ...
    private enum TaskType {
         ...
         *
         * @param type task type
         * @param wifi {@code true} if the task is uploaded using WiFi,
         * {@code false} if Bluetooth is employed
    ...
    private class Task {
         ...
         *
         * @param ownerID ID of the owner of the task
         * @param timestamp timestamp when the task was created
         * @param expiration time when the task expires
         ...
         *
         * @param device type of device executing this task
         * @param sampleTime sample time of the trace
         * @param executorID the ID of the node executing this task
         ...
         *
         * @param device device executing the task
         ...
         * if it hasn't started, or if it has started but hasn't finished
         */
        public boolean hasFinished() {
