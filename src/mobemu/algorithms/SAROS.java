...
/**
 * Class for the SAROS (Social Aware Reputation mechanism for Opportunistic
...
public class SAROS extends Node {
    ...
    /**
     * SAROS values received from other nodes (a[i][j] represents node's i
     * opinion of node j, as known by the current node).
     */
    ...
    private int messageAccepted(int id, boolean source) {
        ...
        /*
         * if a message was pending and its source is encountered, then it is
         * certain that the version it carries is the correct one, so the trust
         * of all the nodes that have relayed the correct version is increased,
         * and the trust of those that have not relayed it correctly is
         * decreased
         */
        if (source) {
    ...
    private void epidemicDataExchange(Node encounteredNode, long contactDuration, long currentTime) {
        ...
        /**
         * if the encountered node doesn't trust me, it doesn't give me its
         * messages
         */
