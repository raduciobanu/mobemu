...
public abstract class Node {
    ...
    public void run(Node encounteredNode, long tick, long contactDuration, boolean newContact, long timeDelta, long sampleTime) {
        ...
        updateBetweennessUtility(timeDelta);
        updateSocialProximity(timeDelta);
     ...
     * current moment
     */
    protected void updateBetweennessUtility(long timeDelta) {
    	
        // if a new window has started, recompute the betweenness utility.
        if (timeDelta / BetweennessUtility.getTimeWindow() > BetweennessUtility.getLastThreshold()) {
        	for(int i = 0; i < betweennessUtility.betweennessUtilities.length; i++)
        		betweennessUtility.betweennessUtilities[i] = 0;
        	BetweennessUtility.increaseLastThreshold();
        }
    }
    
    protected void computeBetweennessUtility(Node encounteredNode, long timeDelta) {
    	betweennessUtility.betweennessUtilities[encounteredNode.getId()] += 1;
    }
     ...
     * current moment
     */
    protected void updateSocialProximity(long timeDelta) {
        // if a new window has started, recompute the proximity.
        if (timeDelta / SocialProximity.getTimeWindow() > SocialProximity.getLastThreshold()) {
        	
        	// reset frequencies
        	for (int i = 0; i < nodes; i++) {
        		socialProximity.setFrequency(i, 0);
        	}

            if (timeDelta >= 2 * SocialProximity.getTimeWindow()) {
            	for (int i = 0; i < nodes; i++) {
            		double value = 0.5 * socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, i)
            				+ 0.5 * socialProximity.getContactDurationValue(SocialProximity.ContactType.PREVIOUS, i);
            		socialProximity.setContactDurationValue(SocialProximity.ContactType.PREVIOUS, i, value);
            	}
            } else {
            	for (int i = 0; i < nodes; i++) {
            		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, i);
                    socialProximity.setContactDurationValue(SocialProximity.ContactType.PREVIOUS, i, value);
            	}
            }
            
            SocialProximity.increaseLastThreshold();

        	for (int i = 0; i < nodes; i++) {
                socialProximity.setContactDurationValue(SocialProximity.ContactType.CURRENT, i, 0);
        	}
        }
        if (timeDelta < SocialProximity.getTimeWindow()) {
        	for (int i = 0; i < nodes; i++) {
        		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, i);
                socialProximity.setContactDurationValue(SocialProximity.ContactType.CUMULATED, i, value);
        	}
        } else {
        	for (int i = 0; i < nodes; i++) {
        		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.PREVIOUS, i);
                socialProximity.setContactDurationValue(SocialProximity.ContactType.CUMULATED, i, value);
        	}
        }
    }
