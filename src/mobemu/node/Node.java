...
public abstract class Node {
    ...
    protected int nodes; // number of nodes in the curent trace
    ...
    // betweenness data
    protected BetweennessUtility betweennessUtility; // the node's betweenness utility value for a given destination
    
    // social proximity data
    protected SocialProximity socialProximity; // the node's social proximity value for a given destination.
    ...
    public Node(int id, int nodes, Context context, boolean[] socialNetwork, int dataMemorySize, int exchangeHistorySize, long seed, long traceStart, long traceEnd) {
        ...
        this.betweennessUtility = new BetweennessUtility(nodes);
        this.socialProximity = new SocialProximity(nodes);
    ...
    public static List<Message> runTrace(Node[] nodes, Trace trace, boolean batteryComputation, boolean dissemination, long seed) {
        ...
        System.out.println("Messages<destination, count>");
        HashMap<Integer, Integer> totalDelivers = new HashMap<>();
        for (Message m : messages) {
			Integer value = totalDelivers.get(m.destination);
			if (value == null) {
				totalDelivers.put(m.destination, 1);
			}
			else {
				totalDelivers.put(m.destination, value+1);
			}
        }
        System.out.println(totalDelivers);
    ...
    public int getBetweennessUtility(int id) {
    	return betweennessUtility.getValue(id);
    }
    
    public double getSocialProximity(int id) {
    	return socialProximity.getFrequency(id) * socialProximity.getContactDurationValue(SocialProximity.ContactType.CUMULATED, id) / SocialProximity.getTimeWindow();
    }
    ...
    public void run(Node encounteredNode, long tick, long contactDuration, boolean newContact, long timeDelta, long sampleTime) {
        ...
        if (newContact) {
            ...
            updateBetweennessUtility(encounteredNode, timeDelta);
            updateSocialProximity(encounteredNode, timeDelta);
            //encounteredNode.updateBetweennessUtility(this, timeDelta);
            //encounteredNode.updateSocialProximity(this, timeDelta);
    ...
    protected void exchangeData(Node encounteredNode, long contactDuration, long currentTime) {
        ...
        if (currentStats.getLastExchangeTime() + currentStats.getLastExchangeDuration() <= currentTime) {
            ...
            if (this.battery.canParticipate() && encounteredNode.battery.canParticipate()) {
                ...
                // compute social proximity contact durations
                computeSocialProximity(encounteredNode, contactDuration, currentTime - Node.traceStart);
                encounteredNode.computeSocialProximity(this, contactDuration, currentTime - Node.traceStart);
    ...
    /**
     * Updates the betweenness utility values.
     *
     * @param timeDelta duration between the beginning of the trace and the
     * current moment
     */
    protected void updateBetweennessUtility(Node encounteredNode, long timeDelta) {  	
        // if a new window has started, recompute the betweenness utility.
        if ((int)(timeDelta / BetweennessUtility.getTimeWindow()) > (int)(betweennessUtility.getLastThreshold(encounteredNode.getId()))) {
			betweennessUtility.setValue(encounteredNode.getId(), 0);
			//betweennessUtility.increaseLastThreshold(encounteredNode.getId());		SET IN LOC DE INCREASE WTF  
			betweennessUtility.setLastThreshold(encounteredNode.getId(), timeDelta / BetweennessUtility.getTimeWindow());
        }
    }
    
    protected void increaseBetweennessUtility(Node encounteredNode, long timeDelta) {
    	int value = betweennessUtility.getValue(encounteredNode.getId());
    	betweennessUtility.setValue(encounteredNode.getId(), value + 1);
    }
    
    /**
     * Updates the social proximity values.
     *
     * @param timeDelta duration between the beginning of the trace and the
     * current moment
     */
    protected void updateSocialProximity(Node encounteredNode, long timeDelta) {
        // if a new window has started, recompute the proximity.
        if ((int)(timeDelta / SocialProximity.getTimeWindow()) > (int)(socialProximity.getLastThreshold(encounteredNode.getId()))) {    	
        	socialProximity.setFrequency(encounteredNode.getId(), 1);    	
            if (timeDelta >= 2 * SocialProximity.getTimeWindow()) {
        		double value = 0.5 * socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId())
        				+ 0.5 * socialProximity.getContactDurationValue(SocialProximity.ContactType.PREVIOUS, encounteredNode.getId());
        		socialProximity.setContactDurationValue(SocialProximity.ContactType.PREVIOUS, encounteredNode.getId(), value);
            } else {
        		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId());
                socialProximity.setContactDurationValue(SocialProximity.ContactType.PREVIOUS, encounteredNode.getId(), value);
            }           
            
            //socialProximity.increaseLastThreshold(encounteredNode.getId(), timeDelta / SocialProximity.getTimeWindow());          	SET IN LOC DE INCREASE WTF  
            socialProximity.setLastThreshold(encounteredNode.getId(), timeDelta / SocialProximity.getTimeWindow());
            socialProximity.setContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId(), 0.0);
        }
        if (timeDelta < SocialProximity.getTimeWindow()) {
    		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId());
            socialProximity.setContactDurationValue(SocialProximity.ContactType.CUMULATED, encounteredNode.getId(), value);
        } else {
    		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.PREVIOUS, id);
            socialProximity.setContactDurationValue(SocialProximity.ContactType.CUMULATED, encounteredNode.getId(), value);
        }
    }
    
    /**
     * Computes the social proximity for the current node upon a contact.
     *
     * @param encounteredNode node the current node is in contact with
     * @param contactDuration contact duration
     * @param timeDelta duration between the beginning of the trace and the
     * current moment
     */
    protected void computeSocialProximity(Node encounteredNode, long contactDuration, long timeDelta) {
    	// increase encounter frequency for given node
    	socialProximity.increaseFrequency(encounteredNode.getId()); 	
    	// increase encounter duration for given node
    	socialProximity.increaseContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId(), contactDuration);  	
    	if (timeDelta < SocialProximity.getTimeWindow()) {
    		double value = socialProximity.getContactDurationValue(SocialProximity.ContactType.CURRENT, encounteredNode.getId());
    		socialProximity.setContactDurationValue(SocialProximity.ContactType.CUMULATED, encounteredNode.getId(), value);
    	}
